...
Pi Space starts the Pi coding agent as a local process. Pi's built-in tools and installed extensions run with the permissions of the operating-system account that launched the app. They can read and modify files, execute commands, use available credentials, and access the network within those permissions.
...
Pi owns provider credentials and configuration under `~/.pi/agent/` on macOS and `%USERPROFILE%\.pi\agent\` on Windows. Pi Space's provider settings update Pi's `models.json` through the native host; the Windows adapter uses atomic replacement and a current-user-only ACL. Pi Space also stores `pi-space-instructions.txt` in Pi's agent directory for its custom-instructions setting. Never commit that directory or share its contents publicly.

The optional Wake Pi Listener continuously processes microphone input while active and authorized on macOS. Pause or quit it from the menu bar when it is not needed. Speech recognition behavior and data handling are governed by macOS and the user's system settings. Voice, wake phrases, and Kokoro are not implemented in the Windows preview.
...
Include the affected revision, operating-system version, reproduction steps, impact, and any suggested mitigation. Reports about Pi itself should be sent to the upstream Pi project according to its security policy.
